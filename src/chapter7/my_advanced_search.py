...
class MyAdvancedSearchTool:
    ...
    def search(self, query: str) -> str:
        ...
        for search_source in self.search_sources:
            try:
                if search_source == "tavily":
                    result = self.search_with_tavily(query)
                    if not result or str(result).startswith("未找到"):
                        return f"📊 Tavily AI搜索结果:\n\n{result}"
                elif search_source == "serpapi":
                    result = self._search_with_serpapi(query)
                    if result and "未找到" not in result:
                        return f"🌐 SerpApi Google搜索结果:\n\n{result}"
            except Exception as e:
                print(f"⚠️ {search_source} 搜索失败: {e}")
                continue
